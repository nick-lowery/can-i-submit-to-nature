---
title: "Can I publish this in Nature? An NLP approach"
subtitle: "Article scraping and exploratory analysis"
date: 2018-11-06
...
```{r frequency, messages = FALSE, warning = FALSE}
library(tidytext)
...
```{r stem frequencies, include = FALSE}

# Since there appear to be some duplication in word forms (article, article's) as well as gene name alphabet soup, what do these plots look like if we stem the words first?

# Answer: not very interesting

stem.freq <- nature.final %>%
  unnest_tokens(word, article) %>%
  anti_join(stop_words, by = "word") %>%
  mutate(stem = SnowballC::wordStem(word)) %>%  # stemming step!
  count(journal, stem, sort = TRUE) %>%
  group_by(journal) %>%
  mutate(freq = n / sum(n)) %>%
  filter(freq > 0.00001) %>%
  select(-n) %>%
...
ggplot(stem.freq, aes(x = freq, y = nature, color = abs(nature - freq))) +
  geom_abline(color = "gray40", lty = 2) +
  geom_jitter(alpha = 0.1, size = 2.5, width = 0.3, height = 0.3) +
...
nature.trigram <- nature.final %>%
  unnest_tokens(trigram, article, token = "ngrams", n = 3) %>%
  separate(trigram, c("word1", "word2", "word3"), sep = " ") %>%
  # omit usual stopwords from the list, as that will likely disrupt potentially interesting phrases
  filter(!word1 %in% c(my.stopwords$word)) %>%
  filter(!word2 %in% c(my.stopwords$word)) %>%
  filter(!word3 %in% c(my.stopwords$word)) %>%
  unite(trigram, word1, word2, word3, sep = " ") %>%
  count(journal, trigram) %>%
  bind_tf_idf(trigram, journal, n)
...
nature.trigram %>%
  ...
  top_n(20, tf_idf) %>%
  ungroup() %>%
  ggplot(aes(trigram, tf_idf, fill = journal)) +
  geom_col(show.legend = F) +
  scale_y_continuous(labels = scientific_format(digits = 1)) +
  labs(x = NULL, y = "tf-idf") +
  facet_wrap(~journal, nrow = 1, scales = "free_y") +
